...
public class Board {
    ...
    public BoardValue[][] guessBoard = null;
    public BoardValue[][] gameBoard = null;
     ...
     * mainly for JUNIT testing purpose
     * @return shipArray - returns the array containing all ships
     */
     ...
     *
     * @param gameBoardFromFile - a string array of game board that correspond with values of game board - these values
     *                          based on a file
     */
        ...
        for (int row = 0; row < gameBoardFromFile.length; row++) {
            for (int col = 0; col < gameBoardFromFile[row].length; col++) {
                switch (gameBoardFromFile[row][col]) {
                    ...
                    case "EMPTY":
                        gameBoard[row][col] = BoardValue.EMPTY;
                        break;
                    case "MISS":
                        gameBoard[row][col] = BoardValue.MISS;
                        break;
                    case "SHIP":
                        gameBoard[row][col] = BoardValue.SHIP;
                        break;
                    case "HIT":
                        gameBoard[row][col] = BoardValue.HIT;
                        aShipSunken(row, col);
                        break;
                }
     ...
     *
     * @param boardType Int that will specify whether the game shall display the
